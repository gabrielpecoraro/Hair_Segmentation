...
class Unet(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, nconvs=1):
        ...
        self.multihead_attn = nn.MultiheadAttention(512, num_heads=4, batch_first=True)
    ...
    def forward(self, x):
        ...
        # Multihead Attention on enc5
        N, C, H, W = enc5.size()
        enc4_flat = enc5.view(N, C, H * W).transpose(1, 2)  # Reshape for attention
        attn_output, attn_output_weights = self.multihead_attn(enc4_flat, enc4_flat, enc4_flat)
        attn_output_reshaped = attn_output.transpose(1, 2).view(N, C, H, W)

        # Apply decoder layers and skip connections
        up4 = self.up4(attn_output_reshaped)
